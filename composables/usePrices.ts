...
export const usePrices = () => {
  ...
  console.log(currencyCode.value)
  ...
  const getVariantPrice = (variant: ProductVariant) => {
    return variant.prices.find(
      (p) => p.currency_code.toLowerCase() === currencyCode.value.toLowerCase()
    )
  }
  ...
  const formatPrice = (variant: ProductVariant, digits: number = 2) =>
    variant.prices.length === 0
      ? "No price"
      : formatMoneyAmount(applyTax(getVariantPrice(variant).amount), digits)
